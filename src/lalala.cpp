...
{ 
	...
	fftwf_execute_dft_r2c(plan, &nx[0], (fftwf_complex*)&Nx[0]); fftwf_execute_dft_r2c(plan, &ny[0], (fftwf_complex*)&Ny[0]); 
	fftwf_execute_dft_r2c(plan, &dx[0], (fftwf_complex*)&Dx[0]); fftwf_execute_dft_r2c(plan, &dy[0], (fftwf_complex*)&Dy[0]); 
...
void reconstruct_height_map1(const float *normal, int width, int height, float *result) 
{
	...
	dx[0] = 1, dx[1] = -1; dy[0] = 1, dy[width] = -1; 
...
void reconstruct_height_map2(const float *normal, int width, int height, float *result) {
	std::vector<float> dx(width*height), dy(width*height); dx[width-1] = 1, dx[1] = -1; 
	dy[width*(height-1)] = 1, dy[width] = -1; reconstruct_height_map(normal, &dx[0], &dy[0], width, height, result); 
} 
*/
//See more at: http://compgroups.net/comp.graphics.api.opengl/bump-map-from-normal-map/489868#sthash.SgRCMzsC.dpuf