...
public:
	...
	static void HsbAdjust	(unsigned char* input, unsigned char* output, unsigned width, unsigned height, float modHue, float modSaturation, float modBrightness);
...
public:
	...
	static void brightCont	(ofTexture& tex, ofFbo& out_fbo, int brightness, int contrast);
	
	static void gamma		(ofTexture& tex, ofFbo& out_fbo, float gamma);
	
	static void threshold	(ofTexture& tex, ofFbo& out_fbo, ofVec3f thresholds);
	
	static void strech		(ofTexture& tex, ofFbo& out_fbo, int max, int min, ofVec3f oMax, ofVec3f oMin);

	static void blur		(ofTexture& tex, ofFbo& out_fbo, Kernel& kernel);

	static void edge		(ofTexture& tex, ofFbo& out_fbo, const int  hKernel[], const int  vKernel[], const int dKernel[] = null_kernel, 
							 EdgeOutput output = THRESHOLD_VALUE, int threshold = 0.0f, ofVec3f greyConvertion = ofVec3f(0.33, 0.33, 0.33));

	static void morph		(ofTexture& tex, ofFbo& out_fbo, Kernel& kernel, int kernelCount);
	...
	static void heightmap	(ofTexture& base, ofTexture& invert, ofFbo& out_fbo, float strength, ofVec3f greyConvertion = ofVec3f(0.33, 0.33, 0.33));
	
	static void n2h			(ofTexture& base, ofFbo& out_fbo, bool doX = true);
