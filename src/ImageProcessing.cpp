...
{
	output.allocate(input.getWidth(), input.getHeight(), OF_IMAGE_COLOR);
	HsbAdjust(input.getPixels(), output.getPixels(), input.getWidth(), input.getHeight(), modHue, modSaturation, modBrightness);
}
void ColorProcessing::HsbAdjust(unsigned char* input, unsigned char* output, unsigned width, unsigned height, float modHue, float modSaturation, float modBrightness)
{
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++)
		{
			unsigned char r, g, b;
			r = input[(y * width + x) * 4 + 0];
			g = input[(y * width + x) * 4 + 1];
			b = input[(y * width + x) * 4 + 2];
			...
			output[(y*width + x) * 4 + 0] = color.r;
			output[(y*width + x) * 4 + 1] = color.g;
			output[(y*width + x) * 4 + 2] = color.b;
		}
	}
}
...
void GPUProcessing::brightCont(ofTexture& tex, ofFbo& out_fbo, int brightness, int contrast)
{
	ofShader    shader;
...
void GPUProcessing::gamma(ofTexture& tex, ofFbo& out_fbo, float gamma)
{
	ofShader    shader;
...
void GPUProcessing::threshold(ofTexture& tex, ofFbo& out_fbo, ofVec3f thresholds)
{
	ofShader    shader;
	...
	if (shader.isLoaded()) {
		...
		shader.setUniform3f("threshold", thresholds);
...
void GPUProcessing::strech(ofTexture& tex, ofFbo& out_fbo, int max, int min, ofVec3f oMax, ofVec3f oMin)
{
	ofShader    shader;
...
void GPUProcessing::blur(ofTexture& tex, ofFbo& out_fbo, Kernel& kernel)
{
	ofShader    shader;
...
void GPUProcessing::edge(	ofTexture& tex, ofFbo& out_fbo, const int  hKernel[], const int  vKernel[], const int dKernel[],
...
void GPUProcessing::morph(ofTexture& tex, ofFbo& out_fbo, Kernel& kernel, int kernelCount)
{
	ofShader    shader;
...
void GPUProcessing::heightmap(ofTexture& base, ofTexture& invert, ofFbo& out_fbo, float strength, ofVec3f greyConvertion)
{
	ofShader    shader;
...
void GPUProcessing::n2h(ofTexture& base, ofFbo& out_fbo, bool doX)
{
	ofShader    shader;
