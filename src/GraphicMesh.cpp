...
void GraphicMesh::drawNormals() 
{
	for (int i = 0; i < mesh.getVertices().size(); i++) {
		ofSetColor(0, 255, 0);
		ofDrawLine(mesh.getVertices()[i], mesh.getVertices()[i] + mesh.getNormals()[i] * 2);
	}
}
...
void GraphicMesh::createMeshFromImage(ofPixels& heightmap, float multiplier, float maxHeight) {
	...
	for (int y = 0; y < h; y++) {
		for (int x = 0; x < w; x++) {
			mesh.addVertex(ofPoint(	x * multiplier, 
									y * multiplier, 
									maxHeight * (float(heightmap[x + y * w]) / 255.0f)));
...
void GraphicMesh::setMeshHeightsFromImage(ofPixels& heightmap, float multiplier, float maxHeight) 
{
	int w = heightmap.getWidth();
	int h = heightmap.getHeight();
